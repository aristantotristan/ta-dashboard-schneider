...
function createTarifView() {
    ...
    <div class="tarif-form">
        <h3>Filter Data Tarif (Membutuhkan Data Historis)</h3>
        <p style="color: #94A3B8;">Menggunakan Supabase API untuk menampilkan laporan.</p>
...
async function fetchTarifData() {
    ...
    try {
        ...
        const { data, error } = await supabase
            .from('daily_meter_summary') // Pastikan nama tabel ini sesuai dengan Supabase
            .select('*')
            .eq('meter_id', id)
        ...
        resultDiv.innerHTML = `<p style="color: #EF4444;">Gagal mengambil data: ${e.message || 'Cek koneksi Supabase.'}</p>`;
    }
}
